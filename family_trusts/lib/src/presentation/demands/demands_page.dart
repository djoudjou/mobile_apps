...
class DemandsPage extends StatelessWidget {
  ...
  const DemandsPage({Key? key,required this.user, this.spouse}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => getIt<DemandsBloc>()..add(DemandsEvent.loadDemands(user.familyId)),
        ),
      ],
                ...
                demandsLoaded: (s) {
                  s.demands.fold(
                    (notificationFailure) => showErrorMessage(
                        LocaleKeys.global_serverError.tr(), context),
                    (_) => null,
                  );
                },
                        ...
                        return state.demands.fold(
                          ...
                          (demands) => <Widget>[
                            DemandsInProgressTab(
                              childrenLookups:
                                  demands.inProgressChildrenLookups,
                            ),
                            DemandsPassedTab(
